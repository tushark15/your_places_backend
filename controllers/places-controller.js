...
const getPlaceById = async (req, res, next) => {
  ...
  if (!place) {
    // return res
    //   .status(404)
    //   .json({ message: "Could not find the place with provided id." });
    const error = new HttpError(
...
const getPlacesByUserId = async (req, res, next) => {
  ...
  if (!userWithPlaces || userWithPlaces.places.length === 0) {
    // return res
    //   .status(404)
    //   .json({ message: "Could not find the place with provided user id." });
...
const deletePlace = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError(
      "Something went wrong, could not delete place try catch.",
      500
    );
    return next(error);
  }
